...
> tree-based navigation when using enums. See [Enum state](TreeBasedNavigation.md#Enum-state) for more
> information.
...
Continue reading into [Integration](StackBasedNavigation.md#Integration) for more information on that.
...
dependency management system (see [Dependencies](DependencyManagement.md)) using `DismissEffect`:
...
quite easy. Further, using "non-exhaustive testing" (see [Non exhaustive testing](TestingTCA.md#Non-exhaustive-testing)) can 
...
As mentioned in [Integration](StackBasedNavigation.md#Integration), `StackState` automatically manages IDs
...
sure that all effects finish by the end of the test (see [Testing](TestingTCA.md) for more info).
...
[Non exhaustive testing](TestingTCA.md#Non-exhaustive-testing) for more info).
