...
`SharedReaderKey` protocol. See [Read only shared state](SharingState.md#Read-only-shared-state) for more 
information.
...
[Testing](TestingTCA.md)), because references cannot be copied and so one cannot inspect the changes 
...
This means that if we altered the `SimpleFeature` of the [Testing shared state](SharingState.md#Testing-shared-state) 
...
[Testing gotchas](TestingTCA.md#Testing-gotchas) of the [Testing](TestingTCA.md) article, and a similar quirk
exists for Xcode previews and is discussed below in [Gotchas of Shared](SharingState.md#Gotchas-of-Shared).

The most robust workaround to this issue is to simply not execute your app's entry point when tests
are running, which we detail in [Testing host application](TestingTCA.md#Testing-host-application). This makes it so that you
...
And this technique also works on [custom persistence]([Custom persistence](SharingState.md#Custom-persistence))
strategies.
...
## Gotchas of There are a few gotchas to be aware of when using shared state in the Composable Architecture.
